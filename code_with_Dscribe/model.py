...
class EnergyForceModel(nn.Module):
    def __init__(self, hidden_size,allelements,cutoff,n_max,l_max,debug=False):
        ...
        self.soaplayer = SOAP(
            ...
            n_max=n_max,
            l_max=l_max,)
        desc_dim= self.soaplayer.get_number_of_features()
        ...
        self.energy_net = nn.Sequential(
            nn.Linear(desc_dim, hidden_size),
    ...
    def forward(self, data,cal_force=True):
        ...
        for i in range(batch_size):
            # 使用mask过滤掉padding的原子
            valid_indices = torch.where(mask[i] > 0)[0].cpu().numpy()
            valid_positions = positions.detach().cpu().numpy()[i][valid_indices]
            valid_elements = elements.detach().cpu().numpy()[i][valid_indices]
            atoms_list.append(ase.Atoms(positions=valid_positions, numbers=valid_elements, pbc=False))
        # Calculate SOAP derivatives for each atom in the batch
        dD_dr_batch, soap_features_batch = self.soaplayer.derivatives(
            atoms_list,
            method="analytical"
        )
        
        # Get max number of atoms in this batch
        max_atoms = atom_num  # We already know the max atoms from the padded input
        
        # Process and pad SOAP features
        soap_features_list = [torch.tensor(feat, dtype=torch.float32, requires_grad=True, device=torch.get_default_device()) 
                             for feat in soap_features_batch]
        soap_features = torch.nn.utils.rnn.pad_sequence(soap_features_list, batch_first=True).to(device_for_model)
        
        # Process and pad derivatives
        dD_dr_padded = []
        for i in range(batch_size):
            N = torch.sum(mask[i]).int().item()  # Number of actual atoms in this molecule
            # Pad dD_dr: [N, N, 3, D] -> [max_atoms, max_atoms, 3, D]
            dD_dr_pad = torch.nn.functional.pad(
                torch.tensor(dD_dr_batch[i], dtype=torch.float32, device=torch.get_default_device()), 
                (0, 0, 0, 0, 0, max_atoms - N, 0, max_atoms - N)
            )  # [max_atoms, max_atoms, 3, D]
            dD_dr_padded.append(dD_dr_pad)
        
        dD_dr = torch.stack(dD_dr_padded).to(device_for_model)
        
        soap_dim = soap_features.shape[-1]
        ...
        if cal_force:
            ...
            dE_dD = torch.autograd.grad(total_energy, soap_features, create_graph=True,grad_outputs=torch.ones_like(total_energy))[0]
            #(Batch_N*Atom_N,soap_dim) to (Batch_N,Atom_N,soap_dim)
            dE_dD = dE_dD.view(batch_size,atom_num,soap_dim)
            force = -torch.einsum('bmd,bnmrd->bnr', dE_dD, dD_dr)
            if self.debug:
